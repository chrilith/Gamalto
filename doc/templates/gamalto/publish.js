...
function buildNav(members) {
    ...
    if (members.globals.length) {
        var _find = function(filter) {
            return _.filter(members.globals, function(obj) { return obj.kind == filter }) || [];
        };
        ...
        nav.push({
            ...
            longname: "global",
            name: "global",
            members: _find('member'),
            methods: _find('function'),
            typedefs: _find('typedef'),
            events: _find('event')
        });
    }
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    for (var longname in helper.longnameToUrl) {
        if ( hasOwnProp.call(helper.longnameToUrl, longname) ) {
            ...
            if (myClasses.length) {
                generate('Framework Object: ' + myClasses[0].name, myClasses, helper.longnameToUrl[longname]);
            }
